...
public class HciCloudFuncHelper extends HciCloudHelper {
   ...
   * 显示结果集合
   */
  private static void showRecogResultResult(HwrRecogResult recogResult, TextView resultView) {
    String strResult = "";
    if (recogResult != null) {
    ...
    if (strResult.length() >= 1) {
      resultView.append( strResult.substring( 0, 1 ) +" ");
    }
  }
   ...
   * 开始识别，此方法为非实时识别
   */
  public static void Recog(Context context, String capkey, HwrConfig recogConfig, short[] strokes,
      TextView resultView) {
    int errCode = -1;
    ...
    if (bRet != -1) {
      ...
      if (HciErrorCode.HCI_ERR_NONE == errCode) {
        ...
        showRecogResultResult( recogResult, resultView );
        //Toast.makeText( context, RecogResultResultToString( recogResult ), Toast.LENGTH_SHORT )
        //    .show();
      } else {
        ShowMessage( "hciHwrRecog error:" + HciCloudSys.hciGetErrorInfo( errCode ) );
      }
  ...
  private static String RecogResultResultToString(HwrRecogResult recogResult) {
    String strResult = "";
    if (recogResult != null) {
      ArrayList<HwrRecogResultItem> recogItemList = recogResult.getResultItemList();
      for (int index = 0; index < recogItemList.size(); index++) {
  ...
  public static void Func(Context context, String capkey, TextView view, TextView resultView,
      short[] sShortData) {
    setContext( context );
    setTextView( view );
    ...
    //非实时识别
    Recog( context, capkey, recogConfig, sShortData, resultView );
